...
class NNMultiTrainer:
    ...
    def optimize(self, input_field: LightField, target_field: LightField, kernel: LightField, iterations: int = 100):
        propagator = prop.MultiparameterNNPropagation(self.params)
        self.model = propagator.get_field_modifier()

        logging.info("Model compiling")
        self.model.compile(
            optimizer=keras.optimizers.Adam(learning_rate=6e-2),
            loss=self.amplitudeMSE,
        )
        ...
        logging.info("Fitting model to data")
        self.history = self.model.fit(
            [propagator.get_field_distribution(input_field), propagator.get_field_distribution(kernel)],
            propagator.get_field_distribution(target_field),
            batch_size=1,
            epochs=iterations,
            callbacks=[model_checkpoint_callback],
