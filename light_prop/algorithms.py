...
class GerchbergSaxton:
    ...
    def optimize(
        self, input_field: LightField, target_field: LightField, iterations: int
    ):
        ...
        for _ in range(iterations):
            ...
            input_plane = prop.ConvolutionPropagation(self.params).propagate(
                output_plane
            )
            input_plane.amplitude = input_field.amplitude
            self.params.distance *= -1
            output_plane = prop.ConvolutionPropagation(self.params).propagate(
                input_plane
            )
