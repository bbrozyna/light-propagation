...
class TestLightField:
    ...
    def sample_amp_phase(self):
        ...
        wavelength = 1
        pixel = 0.1
        return LightField(amp, phase, wavelength, pixel)
    ...
    def test_lightfield_from_polar(self):
        ...
        lf = LightField(amp_correct, phase_correct, wavelength, pixel)

        assert compare_np_arrays(lf.get_amplitude(), amp_correct)
        assert compare_np_arrays(lf.get_phase(), phase_correct)

        with pytest.raises(Exception, match="Dimensions do not match"):
            LightField(amp_correct, phase_incorrect, wavelength, pixel)
    ...
    def test_lightfield_conversion(self, sample_amp_phase):
        ...
        lf = sample_amp_phase
        assert compare_np_arrays(lf.get_amplitude(), correct_abs)
    ...
    def test_lightfield_add(self, sample_amp_phase):
        ...
        res = lf1 + lf2
        assert compare_np_arrays(res.get_amplitude(), lf1.get_amplitude() * 2)
    ...
    def test_lightfield_sub(self, sample_amp_phase):
        ...
        lf2 = sample_amp_phase
        res = lf1 - lf2
        assert compare_np_arrays(res.get_amplitude(), np.array([[0, 0], [0, 0]]))
    ...
    def test_lightfield_mul(self, sample_amp_phase):
        ...
        res = lf1 * lf2
        res2 = lf2 * lf1
        assert compare_np_arrays(res.get_amplitude(), lf1.get_amplitude() * lf2.get_amplitude())
        
        assert compare_np_arrays(res2.get_amplitude(), lf1.get_amplitude() * lf2.get_amplitude())
