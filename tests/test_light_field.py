...
class TestLightField:
    ...
    def test_lightfield_init(self):
        ...
        assert self.compare_np_arrays(lf.amplitude, amp_correct)
        assert self.compare_np_arrays(lf.phase, phase_correct)
    ...
    def test_light_field_sub(self, sample_amp_phase):
        ...
        lf2 = sample_amp_phase
        res = lf1 - lf2
        assert self.compare_np_arrays(res.amplitude, np.array([[0,0],[0,0]]))
        assert self.compare_np_arrays(res.phase, lf1.phase)
    ...
    def test_light_field_mul(self, sample_amp_phase):
        ...
        res = lf1 * lf2
        res2 = lf2 * lf1
        assert self.compare_np_arrays(res.amplitude, lf1.amplitude * lf2.amplitude)
        assert self.compare_np_arrays(res.phase, lf1.phase + lf2.phase)
        assert self.compare_np_arrays(res2.amplitude, lf1.amplitude * lf2.amplitude)
        assert self.compare_np_arrays(res2.phase, lf1.phase + lf2.phase)
