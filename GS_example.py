...
if __name__ == "__main__":
    ...
    target = np.array(
        [
            [
                gaussian(np.sqrt((x - x_shift1) ** 2 + y**2), params.sigma)
        ...
        [
            [
                gaussian(np.sqrt((x - x_shift2) ** 2 + y**2), params.sigma)
                for x in np.arange(-params.matrix_size / 2, params.matrix_size / 2)
                * params.pixel
            ]
            for y in np.arange(-params.matrix_size / 2, params.matrix_size / 2)
    ...
    plotter = GeneratePropagationPlot(
        res[0], output_type=GeneratePropagationPlot.PLOT_PHASE
    )
    plotter.save_output_as_figure("outs/structure.png")
    plotter.show()

    plotter = GeneratePropagationPlot(
        res[0], output_type=GeneratePropagationPlot.PLOT_ABS
    )
    plotter.save_output_as_figure("outs/input_field.png")

    plotter = GeneratePropagationPlot(
        res[1], output_type=GeneratePropagationPlot.PLOT_ABS
    )
